...
class FmPhpFileCache
{
	function setFile($originalPath,$cacheLifeTime = false,$public = false,$filename = null)
	{
		...
		{
			if (file_exists($file = FmConfig::$config['cache']['var_private'].FM_SITE_DIR."phpfilecache/".sha1($originalPath).'.php'))
			{
				include $file;
				@unlink($data[2]);
			}
			$cacheDir = FmConfig::$config['cache'][($cacheLifeTime===null?($public?'static_public':'static_private'):($public?'var_public':'var_private'))].FM_SITE_DIR."phpfilecache/";
			...
			if ($cacheLifeTime===false)
				$cacheLifeTime = FmConfig::$config['cache']['file_lifetime'];
	...
	{
		if (file_exists($file = FmConfig::$config['cache']['var_private'].FM_SITE_DIR."phpfilecache/".sha1($id).'.php'))
		{
			include $file;
			@unlink($data[2]);
		}
	...
	{
		_deleteDir(FmConfig::$config['cache'][($cleanStatic?'static':'var').'_private'].FM_SITE_DIR."phpfilecache/");
		_deleteDir(FmConfig::$config['cache'][($cleanStatic?'static':'var').'_public'].FM_SITE_DIR."phpfilecache/");
	}
}