...
class FmPhpRoute
{
	...
	function __update()
	{
		if (!_clear('route') && is_array($routes = FmCache::get('phproute','cached_routes')))
		{
			list(FmPhpRoute::$path,FmPhpRoute::$inverse,FmPhpRoute::$regex) = $routes;
		}
		else
		{			
			if (!is_array($__path = FmCache::get('phproute','static_routes')))
				$__path = array();
			...
			foreach ($__path as $url=>$route)
			{
				...
				foreach ($route as $key=>$value)
					...
					elseif (!is_string($value) && is_numeric($value) && in_array("$value",FmHeader::$statusCodes))
						$__status = $value;
			...
			FmPhpRoute::$path    = $__path;
			FmPhpRoute::$inverse = $__inverse;
			FmPhpRoute::$regex   = $__regex;
			
			FmCache::set('phproute','cached_routes',array(FmPhpRoute::$path,FmPhpRoute::$inverse,FmPhpRoute::$regex),FmConfig::$config['route']['cache_lifetime']);
		}
	}
	...
	function getUrl($view,$arguments = array(),$decorators = array())
	{
		...
		if (isset(FmPhpRoute::$inverse[$view]))
		{
			$routes = array();
			foreach (FmPhpRoute::$inverse[$view] as $url=>$routeData)
			{
				$ok = true;
		...
		if (FmConfig::$config['route']['magic_route']==true && preg_match('/^('.FmConfig::$config['route']['magic_view'].')$/',$view))
		{
			$urlArgs = array();
			$queryArgs = array();
	...
	{	
		...
		if (!_clear('route') && is_array($route = FmCache::get('phproute',$routeKey)))
			return $route;
		else
		{
			$matches = array();
			...
			$path_keys = array();
			foreach (array_keys(FmPhpRoute::$regex) as $regex)
				if (preg_match($regex,$url) && preg_match('#^('.FmPhpRoute::$regex[$regex][2].')$#',$extension))
					$path_keys[] = $regex; 
			...
			{
				list($view,$status,$extensions,$urlArguments,$routeArguments,$route,$regex) = FmPhpRoute::$regex[$path_keys[0]];
			...
			elseif (FmConfig::$config['route']['magic_route']==true && $url!='/' && is_array($args = explode('/',$url)) && preg_match('/^('.FmConfig::$config['route']['magic_view'].')$/',$args[1]))
			{
				array_shift($args);
			...
			{
				$view = FmConfig::$config['route']['404_route'];
				$status = 404;
				$args = explode('/',$url);
				array_shift($args);
			...
			FmCache::set('phproute',$routeKey,$route,FmConfig::$config['route']['cache_lifetime']);
